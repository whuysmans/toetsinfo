...
const buildResultTable = ( restData, graphqlData ) => {
	...
	const tableBody = `<tbody>
		...
		<tr class="hover-container"><td>${ questionTypes.description }</td><td>${ questionTypes.value }</td><td style="background-color:orange";>OK?</td><td class="hover-target">&#9432;<aside class="hover-popup"><p>${ questionTypes.value !== '' ? 'Controleer of al soortern examenvragen die je in je examen wil openmen hier vermeld staan.' : 'Er zitten nog geen vragen in de toets. Voeg deze zeker nog toe! Als je met toetsbanken werkt: controleer of de linken naar de banken goed gelegd zijn.' }</p></aside></td></tr>
		...
		<tr class="hover-container"><td>${ hasAccessCode.description }</td><td>${ hasAccessCode.value === 'true' ? 'Ja' : 'Nee' }</td><td style="background-color:${ rowColor(hasAccessCode) }">${ hasAccessCode.value === hasAccessCode.expectedValue ? 'OK' : 'NOK' }</td><td class="hover-target">&#9432;<aside class="hover-popup"><p>Stel de correcte einddatum en -tijd in. Controleer zeker of je de beschikbaarheid 1 uur langer dan de duurtijd van het examen hebt ingesteld.</p></aside></td></tr>
		...
		${ !isMIT() ? '' : `<tr class="hover-container"><td>${ isInModule.description }</td><td>${ isInModule.value }</td><td style="background-color:${ isInModule.value === 'Nee' ? 'red' : 'orange' }">${ isInModule.value === 'Nee' ? 'NOK' : 'OK?' }</td><td class="hover-target">&#9432;<aside class="hover-popup"><p>Als je niet bij MIT werkt, mag je dit negeren! Als je wel bij MIT werkt, controleer dan of dit inderdaad de module is waarin de toets moet zitten. </p></aside></td></tr` }
		</tbody>
	`
